...
def load(path, *args):
    """
    Loads the specified data from a folder containing .h5 files and saves it in an array of shape (1, Nbins, -1).
    Nbins is the number of .h5 files in the folder.
    Not yet divided by sign.

    Returns
    -------
        data : tuple
            Tuple of the data requested by *args.
    """
    files = sorted(glob(path + "*.h5"))
    nbins = len(files)
    # print(f"nbins : {nbins}") # for testing
    if nbins == 0:
