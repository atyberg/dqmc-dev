...
def bond_params(
    ...
    """Geometry specific 1-bond parameters 
    TODO: triangular, kagome
    TODO: tpp != 0 for all
    TODO: tp != 0 for honeycomb
    ...
    if geometry == "square":
        ...
        bps = 6 if tp != 0.0 else 1.5  # CHECK TP != 0 CASE, TP=0 CASE IS OK
    elif geometry == "kagome":
    ...
    # NOTE: changed the map_b and degen_b from being a general definition for all lattice geometries (in original version)
    # to being geometry-specific.
    ...
    if geometry == "square":
        ...
        # per 1-bond (per SITE) measurement mapping
        # had to change all of this because bps = 1.5, float gives problems... had to use bonds per unit cell instead
        bonds_per_cell = int(bps*Norb_per_cell_dict[geometry])
        num_b = int(num_b) # have to convert bc without it's 3.0 (a float)
        if trans_sym:
            map_b = np.tile(np.arange(bonds_per_cell, dtype=np.int32), (Nx*Ny, 1)).T.flatten()  # N_unit_cells * bonds per unit cell
            degen_b = np.full(bonds_per_cell, Nx*Ny, dtype=np.int32) # N unit cells * bonds per unit cell
        else:
            map_b = np.arange(num_b, dtype=np.int32)  # N*bps
            degen_b = np.ones(num_b, dtype=np.int32)  # length N*bps
        num_b_accum = map_b.max() + 1
        ...
        for y in range(Ny):
            for x in range(Nx):
                # Neighbors for the a atom at (x,y,0)
                i = x + Nx*y + Nx*Ny*0 # general formula i = x + Nx*y + Nx*Ny*orb_ind, where orb_ind = 0 for a atom or 1 for b atom
                # nn within unit cell: end point (x, y, (orb_ind_a+1)%Norb_per_cell) = (x, y, 1)
                bonds[0, i] = i # start point of bond
                bonds[1, i] = x + Nx*y + Nx*Ny*1
                # nn along x lattice vector: end point ((x-1)%Nx, y, 1)
                bonds[0, i + Nx*Ny] = i # start point
                bonds[1, i + Nx*Ny] = (x-1)%Nx + Nx*y + Nx*Ny*1
                # nn along y lattice vector: end point (x, (y-1)%Ny, 1)
                bonds[0, i + 2*Nx*Ny] = i
                bonds[1, i + 2*Nx*Ny] = x + Nx*((y-1)%Ny) + Nx*Ny*1

                # Only counts bonds starting from the A site to not double count any bonds

                if bps == 6: 
                    # NOTE: the number 6 is currently a placeholder, tp != 0 currently not yet implemented
                    raise NotImplementedError
...
def bond2_params(
    ...
    """Geometry specific 2-bond parameters, defined by 2 hopping steps
    TODO: triangular, kagome
    TODO: tpp != 0 for all
    TODO: tp != 0 for honeycomb
    Returns:
    ...
    if geometry == "square":
        ...
        b2ps = 12 if tp != 0.0 else 3  # tp = 0 OK, tp != 0 probably wrong (not implemented)
    elif geometry == "kagome":
        b2ps = 12 if tp != 0.0 else 4  # probably wrong
    else:
        raise NotImplementedError
    ...
    if geometry == "square":
        ...
        for y in range(Ny):
            for x in range(Nx):
                # Bonds from the A atom at (x,y,0)
                ia = x + Nx*y + Nx*Ny*0

                # bond along +x at ((x+1)%Nx, y, 0)
                bond2s[0,ia] = ia
                bond2s[1,ia] = (x+1)%Nx + Nx*y + Nx*Ny*0 
                # bond along +y at (x, (y+1)%Ny, 0)
                bond2s[0,ia + N] = ia
                ...
                # bond along +x at ((x+1)%Nx, y, 1)
                bond2s[0,ib] = ib
                bond2s[1,ib] = (x+1)%Nx + Nx*y + Nx*Ny*1
                # bond along +y at (x, (y+1)%Ny, 1)
                bond2s[0,ib + N] = ib
